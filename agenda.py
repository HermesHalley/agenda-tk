...
def editarContato() -> None:
    ...
    opcao = messagebox.askyesno("Atenção!!!", "Deseja alterar os dados do contato?")
    if opcao:
        agenda[index] = {
            "nome": txt_nome.get(),
            "telefone": txt_telefone.get(),
...
def excluirContato() -> None:
    opcao = messagebox.askyesno("Atenção!!!", "Deseja deletar o contato?")
    if opcao:
...
btn_adicionar = Button(janela, text="Adicionar", fg="navy", font="Tahoma 10 bold", background="white", width=8,
                       command=adicionarContato)
btn_adicionar.grid(row=3, column=0)

btn_editar = Button(janela, text="Editar", fg="orange", font="Tahoma 10 bold", background="white", width=8,
                    command=editarContato)
btn_editar.grid(row=3, column=1)

btn_excluir = Button(janela, text="Excluir", fg="red", font="Tahoma 10 bold", background="white", width=8,
